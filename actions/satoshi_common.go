...
// btc 转账 （amt 单位 聪）
func DoSimpleSatoshiTransferFromChainState(state interfaces.ChainStateOperation, addr1 fields.Address, addr2 fields.Address, amt fields.VarInt8) error {
	...
	amt2 := bls2.Amount
	bls1.Amount = fields.VarInt8(uint64(amt1) - uint64(amt)) // 扣除
	bse1 := state.SatoshiSet(addr1, bls1)
	if bse1 != nil {
		return bse1
	}
	bls2.Amount = fields.VarInt8(uint64(amt2) + uint64(amt)) // 增加
...
func DoAddSatoshiFromChainState(state interfaces.ChainStateOperation, addr fields.Address, amt fields.VarInt8) error {
	...
	// 新余额
	blssto.Amount = fields.VarInt8(newamt)
...
func DoSubSatoshiFromChainState(state interfaces.ChainStateOperation, addr fields.Address, amt fields.VarInt8) error {
	blssto := state.Satoshi(addr)
	baseamt := blssto.Amount
	// 检查余额
